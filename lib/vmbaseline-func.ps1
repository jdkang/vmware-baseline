...
param(
	...
	PROCESS {
		foreach ($vm in $vms) {
			...
			Remove-PSDrive -Name tmpdsmnt -ErrorAction silentlycontinue
			new-psdrive -name tmpdsmnt -location (get-datastore $dsname) -psprovider vimdatastore -root ‘/’ | out-null
			
			$vmxfileName = $vmxPath.split('/')[1]
			$destVmx = ("$saveFolder\") + $vmxfileName
			# copy .vmx
			write-verbose "Copying VMX to $destVmx"
			Copy-DatastoreItem -Item tmpdsmnt:\$vmxPath -Destination $destVmx -Force
